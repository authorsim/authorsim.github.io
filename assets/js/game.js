...
export const researchRequirementsMet = () => {
  ...
  if (save[req.first.unit].total >= req.first.num &&
      save[req.second.unit].total >= req.second.num &&
      save[req.third.unit].total >= req.third.num) {
    console.log('true')
    return true
  }
  return false
...
export const setResearchRequirements = () => {
  ...
  req.first.num = Math.floor(Math.random() * (400000 - 200000 + 1)) + 200000
  req.second.num = Math.floor(Math.random() * (400 - 200 + 1)) + 200
  req.third.num = Math.floor(Math.random() * (4 - 2 + 1)) + 2
}
...
const loadGame = () => {
  if (localStorage.getItem('save') !== null) {
    ...
    */
    ...
    save.encyclopedias = load.encyclopedias
